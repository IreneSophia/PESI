...
fid = fopen(path_dat + "PESI-BV-" + subID + "_" + datestr(datetime(),'yyyymmdd-HHMM') + ".csv", 'w');
...
if eyeTracking
    ...
    crsLiveTrackSetDataFilename(char(path_dat + "PESI-ET-" + subID + "_" + datestr(datetime(),'yyyymmdd-HHMM') + ".csv"));
end
...
%% First run.
    ...
    vids{runs(1)} = Shuffle(vids{runs(1)},2);
...
%% Take a break.
    ...
    % Draw start text.
    DrawFormattedText(window, 'Sie können nun eine Pause machen, sich bewegen oder etwas trinken.\nSagen Sie Bescheid, wenn Sie weitermachen möchten.',...
        'center', yCenter-shift, white);
    ...
    % Close everything and re-enable the keyboard to do a new calibration.
    Screen('Close');
    ShowCursor(window);
    sca;
    ListenChar(1);
...
%% Second run.
    ...
    % Randomise the order of the videos.
    vids{runs(2)} = Shuffle(vids{runs(2)},2);
    
    % Get number of trials. 
    ntrials = length(vids{runs(2)});
    ...
    for j = 1:ntrials
        ...
        % Determine which video is loaded. 
        video = vids{runs(2)}{j,1};
        dyad  = vids{runs(2)}{j,2};
        sync  = vids{runs(2)}{j,3};
        ...
        % Log all the information for this trial
        fprintf(fid, '%s,%i,%i,%s,%s,%s,%.4f,%.2f,%i,%i,%.4f\n',...
            subID,runs(2),j+end_trial,video,dyad,sync,dur,rating,confirmed,moved,rt);
