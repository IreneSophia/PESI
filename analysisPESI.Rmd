...
# Explore the other interactions
e1.1 = hypothesis(m.pesi, "diagnosis1:sync1 < 0")
e1.1
e1.2 = hypothesis(m.pesi, "diagnosis1:sync1:dyad.type1 > 0")
e1.2
...
# H2.1 Dyad type x AOI: Fixation durations for each area of interest differ
# between clips of no-diagnosis and mixed-diagnosis dyads.
h2.1 = hypothesis(m.fix, "(dyad.type1:AOI1 + dyad.type1:AOI2)/2 < 0", alpha = 0.025)
h2.1

# H2.1 Synchrony x AOI: Fixation durations for each AOI differ between 
# high- and low-synchrony clips. 
h2.2 = hypothesis(m.fix, "(sync1:AOI1 + sync1:AOI2)/2 > 0", alpha = 0.025)
h2.2 

# H2.3 Diagnostic status x AOI: Fixation durations for each AOI differ between 
# autistic and non-autistic participants
h2.3 = hypothesis(m.fix, "(diagnosis1:AOI1 + diagnosis1:AOI2)/2 < 0", alpha = 0.025)
h2.3

# exploring which AOI is driving this...
e2.1 = hypothesis(m.fix, "diagnosis1:AOI1 < 0", alpha = 0.025)
e2.1
e2.2 = hypothesis(m.fix, "diagnosis1:AOI2 < 0", alpha = 0.025)
e2.2
e2.3 = hypothesis(m.fix, "-diagnosis1:AOI1 -diagnosis1:AOI2 > 0", alpha = 0.025)
e2.3
...
```{r sbc_checks2_sac, fig.height=12}
...
# set formula
code   = "PESI_com"
f.com = brms::bf(rating.confirmed ~ sn.sac + s.head + s.hand + s.body + (1 | subID) + (1 | dyad))
...
```{r sbc_checks2_com, fig.height=12}
...
## Synchronisation of autistic and non-autistic participants in the mixed dyads

```{r setup_syncronisation}

# we focus on the mixed dyads to compare the influence of synchronisation / leading
# s1 is left, s2 is the right person, we recode this to diagnostic status
df.lead = df %>%
  filter(dyad.type == "mixed") %>%
  mutate(
    ASDpeak = case_when(
      dyad == "D22g" ~ s2peak, 
      dyad == "D24g" ~ s2peak, 
...
# run spearman correlations for ASDpeaks and COMpeaks: https://osf.io/j5wud
...
if (file.exists("ASD_rho.rds")) {
  rhoSamples.asd = readRDS("ASD_rho.rds")
} else {
  rhoSamples.asd = 
    spearmanGibbsSampler(xVals = df.lead$rating.confirmed,
                         yVals = df.lead$ASDpeak, 
                         nSamples = 5e3)
  saveRDS(rhoSamples.asd, file = "ASD_rho.rds")
}

# give the posterior samples for rho to the function below to compute BF01
asdcor.bf = computeBayesFactorOneZero(rhoSamples.asd$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
log(asdcor.bf)

# visualise it
ggplot(data = df.lead, aes(x = rating.confirmed, y = ASDpeak)) +
...
# Default beta prior width is set to a = b = 1 for the sampler 
if (file.exists("COM_rho.rds")) {
  rhoSamples.com = readRDS("COM_rho.rds")
} else {
  rhoSamples.com = 
    spearmanGibbsSampler(xVals = df.lead$rating.confirmed,
                         yVals = df.lead$COMpeak, 
                         nSamples = 5e3)
  saveRDS(rhoSamples.com, file = "COM_rho.rds")
}

# give the posterior samples for rho to the function below to compute BF01
comcor.bf = computeBayesFactorOneZero(rhoSamples.com$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
log(comcor.bf)

# visualise it
ggplot(data = df.lead, aes(x = rating.confirmed, y = COMpeak)) +
  geom_point(colour = "lightblue") +
  geom_smooth(method = "lm", 
              formula = y ~ x, 
              geom = "smooth", colour = c_mid_highlight) +
