...
The PESI project focuses on "Perception of nonverbal social interaction in people with and without autism". Specifically, autistic and non-autistic adult participants watched 64 silent videos of dyadic conversations between two non-autistic or one autistic and one non-autistic adult. These videos were 10 seconds of longer conversations and either portrayed moments of high or low interpersonal synchrony of motion. The videos only showed outlines of the dyad to direct the focus on the movement. During the video presentation, participants' gaze is tracked and after each video they are asked to rate how comfortable they imagine this interaction to be. 
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Diagnostic status: Ratings of social interactions differ between autistic and non-autistic participants. 
    ...
    \item Dyad type x diagnostic status: The effect of dyad type on ratings is decreased in autistic compared to non-autistic participants.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Diagnostic status x AOI: Fixation durations for each area of interest differ between autistic and non-autistic participants. 
  \end{enumerate}
\end{enumerate}
...
if (!file.exists("PESI_data.RData")) {
  ...
  df = df %>% select(subID, diagnosis, dyad, video, run, trl, sync, rating, 
                     dyad.type, mot, peak, rating.confirmed, thre, steep, s1peak, s2peak)
  save(df, df.table, df.sht, ct.full, df.exc, df.incET, df.fix, df.sac.agg, 
...
h1.c = hypothesis(m.pesi, "diagnosis1 > 0")
h1.c
...
Our Bayesian linear mixed model evaluating predictors for the ratings reveals support for our hypotheses postulating decreased ratings for mixed dyads (*Estimate* = `r round(h1.a$hypothesis$Estimate,2)` [`r round(h1.a$hypothesis$CI.Lower,2)`, `r round(h1.a$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h1.a$hypothesis$Post.Prob,3)`) and increased ratings for high synchrony videos (*Estimate* = `r round(h1.b$hypothesis$Estimate,2)` [`r round(h1.b$hypothesis$CI.Lower,2)`, `r round(h1.b$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h1.b$hypothesis$Post.Prob,3)`). However, there is no support for our hypotheses regarding differences between the ratings of autistic and non-autistic participants (*Estimate* = `r round(h1.c$hypothesis$Estimate,2)` [`r round(h1.c$hypothesis$CI.Lower,2)`, `r round(h1.c$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h1.c$hypothesis$Post.Prob,3)`) or the interaction of synchrony and dyad type (*Estimate* = `r round(h1.d$hypothesis$Estimate,2)` [`r round(h1.d$hypothesis$CI.Lower,2)`, `r round(h1.d$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h1.d$hypothesis$Post.Prob,3)`) or dyad type and diagnostic status (*Estimate* = `r round(h1.e$hypothesis$Estimate,2)` [`r round(h1.e$hypothesis$CI.Lower,2)`, `r round(h1.e$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h1.e$hypothesis$Post.Prob,3)`). 
...
df.agg %>%
  ...
  ggplot(aes(dyad.type, rating.confirmed, fill = sync, colour = sync)) + 
...
```{r loo_comparison, eval=F}

# intercept model
m.pesi0 = brm(rating.confirmed ~ 1 + (1 | subID) + (1 | dyad), 
              data = df.agg, prior = priors %>% filter(class != "b"),
              iter = iter, warmup = warm,
              backend = "cmdstanr", threads = threading(8),
              file = file.path(loo_dir, "m_PESI_0"),
              save_pars = save_pars(all = TRUE)
)
# main effect models
m.pesi1 = brm(rating.confirmed ~ diagnosis + (1 | subID) + (1 | dyad), 
              data = df.agg, prior = priors %>% 
                filter(class != "b"),
              iter = iter, warmup = warm,
              backend = "cmdstanr", threads = threading(8),
              file = file.path(loo_dir, "m_PESI_1"),
              save_pars = save_pars(all = TRUE)
)
m.pesi2 = brm(rating.confirmed ~ dyad.type + (1 | subID) + (1 | dyad), 
              data = df.agg, prior = priors %>% 
                filter(class != "b" | coef == "dyad.type1"),
              iter = iter, warmup = warm,
              backend = "cmdstanr", threads = threading(8),
              file = file.path(loo_dir, "m_PESI_2"),
              save_pars = save_pars(all = TRUE)
)
m.pesi3 = brm(rating.confirmed ~ sync + (1 | subID) + (1 | dyad), 
              data = df.agg, prior = priors %>% 
                filter(class != "b" | coef == "sync1"),
              iter = iter, warmup = warm,
              backend = "cmdstanr", threads = threading(8),
              file = file.path(loo_dir, "m_PESI_3"),
              save_pars = save_pars(all = TRUE)
)

# calculate the loos for these models
ls.loo = list(
  loo(m.pesi0),
  loo(m.pesi1),
  loo(m.pesi2),
...
```{r sbc_checks2_fix, fig.height=24}
...
p4 = plot_contraction(
  ...
  prior_sd = setNames(c(11, 
                        rep(11, length(unique(df.results.b$variable))-1)), 
                      unique(df.results.b$variable))) +
  theme_bw() +
  scale_x_continuous(breaks=scales::pretty_breaks(n = 3)) +
  scale_y_continuous(breaks=scales::pretty_breaks(n = 3))
...
e1 = hypothesis(m.fix, "diagnosis1:AOI1 < 0", alpha = 0.025)
e1
e2 = hypothesis(m.fix, "diagnosis1:AOI2 < 0", alpha = 0.025)
e2
e3 = hypothesis(m.fix, "-diagnosis1:AOI1 -diagnosis1:AOI2 > 0", alpha = 0.025)
e3

```

Our second Bayesian linear mixed model evaluating predictors for the proportional dwell times reveals support for our expectation of differences between autistic and non-autistic participants dependent on the specific AOI (*Estimate* = `r round(h2.c$hypothesis$Estimate,2)` [`r round(h2.c$hypothesis$CI.Lower,2)`, `r round(h2.c$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h2.c$hypothesis$Post.Prob,3)`). Further exploration shows that this is driven by reduced dwell times on the head region (*Estimate* = `r round(e1$hypothesis$Estimate,2)` [`r round(e1$hypothesis$CI.Lower,2)`, `r round(e1$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(e1$hypothesis$Post.Prob,3)`) and increased dwell times on the body region (*Estimate* = `r round(e3$hypothesis$Estimate,2)` [`r round(e3$hypothesis$CI.Lower,2)`, `r round(e3$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(e3$hypothesis$Post.Prob,3)`) for autistic participants. 

There is no support for our hypotheses regarding interactions between synchrony and AOI (*Estimate* = `r round(h2.b$hypothesis$Estimate,2)` [`r round(h2.b$hypothesis$CI.Lower,2)`, `r round(h2.b$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h2.b$hypothesis$Post.Prob,3)`) or dyad type and AOI (*Estimate* = `r round(h2.a$hypothesis$Estimate,2)` [`r round(h2.a$hypothesis$CI.Lower,2)`, `r round(h2.a$hypothesis$CI.Upper,2)`], *Posterior probability* = `r round(h2.a$hypothesis$Post.Prob,3)`). 
...
df.fix.agg %>%
  ...
  ggplot(aes(AOI, fix.prop, fill = sync, colour = sync)) + 
  geom_rain(rain.side = 'r', 
            boxplot.args = list(colour = "black", 
                                outlier.shape = NA, 
                                show.legend = FALSE,
  ...
  ylim(0, 100) +
  facet_wrap(dyad.type ~ diagnosis) +
  scale_fill_manual(values = custom.col) +
  scale_color_manual(values = custom.col) +
  labs(title = "Mean fixation duration proportions per subject", 
       x = "", 
       y = "percent") +
  theme_bw() + 
  theme(legend.position = "bottom", 
...
Additionally, we focus on specific interactions. 

```{r plot_fix2, fig.height=4}

# rain cloud plot for fixation durations, focusing on synchrony x AOI
df.fix.agg %>%
  group_by(subID, diagnosis, sync, AOI) %>%
  summarise(
    fix.prop = mean(fix.prop, na.rm = T)
  ) %>%
  ggplot(aes(AOI, fix.prop, fill = sync, colour = sync)) + 
...
# rain cloud plot for fixation durations, focusing on dyad type x AOI
df.fix.agg %>%
  group_by(subID, diagnosis, dyad.type, AOI) %>%
  summarise(
    fix.prop = mean(fix.prop, na.rm = T)
  ) %>%
  ggplot(aes(AOI, fix.prop, fill = dyad.type, colour = dyad.type)) + 
  geom_rain(rain.side = 'r', 
            boxplot.args = list(colour = "black", 
                                outlier.shape = NA, 
                                show.legend = FALSE,
...
# rain cloud plot for fixation durations, focusing on diagnostic status x AOI
df.fix.agg %>%
  group_by(subID, diagnosis, AOI) %>%
  summarise(
    fix.prop = mean(fix.prop, na.rm = T)
  ) %>%
  ggplot(aes(AOI, fix.prop, fill = diagnosis, colour = diagnosis)) + 
  geom_rain(rain.side = 'r', 
            boxplot.args = list(colour = "black", 
                                outlier.shape = NA, 
                                show.legend = FALSE,
...
Our simulated data centres around participants only producing a couple of saccades per video, which is reasonable since the videos are only 10 seconds long. However, there is some data simulated that shows up to 50 saccades, so it is reasonably wide and we are happy with the prior predictive distribution. 

```{r sbc_checks2_sac, fig.height=9}
...
While autistic participants seem to have produced fewer saccades on average, this was independent of the conditions. Additionally, there seem to be no differences associated with the task conditions itself. 
...
```{r sbc_checks2_com, fig.height=9}
...
It seems as if decreased dwell times on the hands was associated with increased ratings. 
