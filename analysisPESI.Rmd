...
if (!file.exists("PESI_data.RData")) {
  ...
  df.fix = merge(df.fix, 
                 df %>% select(video, dyad.type, sync, mot, peak), 
                 all.x = T) %>%
    # add diagnostic group
    merge(., df.inc %>% select(subID, diagnosis) %>% distinct(), all.x = T)
  ...
  # save it all
  df = df %>% select(subID, diagnosis, video, run, trl, sync, rating, dyad.type, 
                     mot, peak, rating.confirmed, thre, steep)
...
boxplot.args = list(color = "black", outlier.shape = NA, 
                    show_guide = FALSE, alpha = 0.5),
violin.args = list(color = "black", outlier.shape = NA, 
                   alpha = 0.5),
...
point.args.pos = list(position = 
                        ggpp::position_dodgenudge(x = -0.25, width = 0.1))) +
  ylim(0, 100) +
